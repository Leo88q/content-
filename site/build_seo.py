...
def main():
    ...
    # «Ганнеры дня» — ежедневная SEO-страница: свежий поисковый контент каждый прогон
    day = now.strftime("%Y-%m-%d")
    gainers = sorted(pools, key=lambda p: (p.get("change") or {}).get("h24") or 0, reverse=True)[:10]
    rows = "\n".join(
        f'<tr><td>{i}</td><td><a href="../pools/{p["address"]}.html">{p.get("base_symbol", "?")}</a></td>'
...
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Топ-ганнеры Solana за 24ч — {day} | TalkChart</title>
<meta name="description" content="Кто растёт на Solana прямо сейчас: топ-10 пулов за 24 часа с автоматическим разбором причин движения. Обновляется фабрикой контента.">
<link rel="canonical" href="{config.SITE_URL}/gainers/latest.html">
</head>
<body style="background:#0a0e14;color:#e8edf2;font-family:monospace;max-width:900px;margin:auto;padding:40px 20px;line-height:1.8">
<p><a href="../index.html" style="color:#7cf03d">📈 TalkChart</a> / ганнеры дня</p>
...
{rows}</table>
<p style="color:#8b98a5">Страница генерируется автоматически каждый прогон фабрики (каждые 4 часа).
<a href="latest.html" style="color:#7cf03d">Свежая версия →</a></p>
</body></html>"""
    gainers_dir = os.path.join(os.path.dirname(config.POOLS_DIR), "gainers")
    os.makedirs(gainers_dir, exist_ok=True)
    with open(os.path.join(gainers_dir, f"{day}.html"), "w") as f:
        f.write(gainers_html)
    with open(os.path.join(gainers_dir, "latest.html"), "w") as f:
        f.write(gainers_html)
    print(f"OK: ганнеры дня {day} -> site/gainers/")
    items = "\n".join(
